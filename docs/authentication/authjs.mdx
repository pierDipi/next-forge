...
Auth.js requires a random value secret, used by the library to encrypt tokens and email verification hashes. Generate a secret with the following command:

```sh Terminal
npx auth secret
```

This will automatically add an `AUTH_SECRET` environment variable to the root `.env.local` file.
...
```tsx apps/app/app/api/auth/[...nextauth]/route.ts
