...
Environment variables are defined in a `.env.local` file in the root of the project and validated in `packages/env/index.ts`. The file exports an `env` object that contains all validated environment variables, separated into `server` and `client` variables.

## Symlinks

The root `.env.local` file will be symlinked to the `.env.local` file in each Next.js application, as well as a `.env` file in the database package. This means that you should only need to update the root `.env.local` file to add or update environment variables for any project in the repo.

<Warning>
  Because the contents of the symlinked `.env.local` files don't technically change, Next.js will not restart your application when you change the root `.env.local` file. You will need to manually restart your application.
</Warning>
...
As part of the initial setup, you will need to fill in the environment variables in the `.env.local` file. Check out the `packages/env/index.ts` file to see the validation rules for each variable.

<Tip>
    For `FLAGS_SECRET`, you can run `node -e
    "console.log(crypto.randomBytes(32).toString('base64url'))"` or `openssl rand -base64 32` in
    your terminal to generate a random value.
</Tip>
...
1. Add the variable to the root `.env.local` file
